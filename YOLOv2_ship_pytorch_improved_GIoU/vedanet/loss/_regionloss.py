...
from .util import bbox_ious,bbox_diou, bbox_giou, bbox_overlaps_ciou
...
class RegionLoss(nn.modules.loss._Loss):
    ...
    # reduction=32
    def __init__(self, num_classes, anchors, anchors_mask, reduction=None, seen=0, coord_scale=1.0, noobject_scale=1.0, object_scale=5.0, class_scale=1.0, thresh=0.6, head_idx=0):
        super().__init__()
        self.num_classes = num_classes
        self.num_anchors = len(anchors_mask)
        self.anchor_step = len(anchors[0])    #5
    ...
    def forward(self, output, target, seen=None):
        ...
        """
        # Parameters
        nB = output.size(0)  #batch  图片数量
        nA = self.num_anchors
        nC = self.num_classes
        nH = output.size(2)  #特征图高
        nW = output.size(3)   #特征图宽
    ...
    def __build_targets_brambox(self, pred_boxes, ground_truth, nH, nW):
        ...
        for b in range(nB):
            ...
            for i, anno in enumerate(ground_truth[b]):   #这张图里的gt
            ...
            iou_gt_pred = bbox_giou(gt, cur_pred_boxes)#通过IOU来确定谁是最佳
            # IOU大于阈值的我们则认为是可以用来预测的了
            # 则它们的conf_neg应该为0，剩下的那些就是neg格子了。
            # print(iou_gt_pred)   #打印输出的内容
            # print(iou_gt_pred.shape)  #打印维度
            # iou_gt_pred_1 = iou_gt_pred.unsqueeze(0)  #增加一个维度，原来为（[588]）,增加后维度为([1, 588])

            # print(iou_gt_pred_1)
            # print(iou_gt_pred_1.shape)
            mask = (iou_gt_pred > self.thresh).sum(0) >= 1
            # print(mask)
            conf_neg_mask[b][mask.view_as(conf_neg_mask[b])] = 0
            #mask.view_as(conf_neg_mask[b])  将mask转为与conf_neg_mask[b]的维度大小相同的tensor
            # Find best anchor for each gt
            # 然后计算了GT和5个anchor中最接近的一个
            gt_wh = gt.clone()
            gt_wh[:, :2] = 0
            # iou_gt_anchors = bbox_ious(gt_wh, anchors)
            iou_gt_anchors = bbox_giou(gt_wh, anchors)
            # iou_gt_anchors_1 = iou_gt_anchors.unsqueeze(0)
