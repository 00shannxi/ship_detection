...
#19558332860469818}]}
    ...
    # voc_wrapper.genResults(reorg_dets, results, nms_thresh)
        for k, v in det.items():  #遍历字典   k为图片  v为检测结果   遍历一个batch里的16张图  一次遍历一幅图
            ...
            name = k[79:-4]   #表示只保留006971， 79表示F:\pycharm\YOLOv2_ship_pytorch\data\Seaships_7000\VOCdevkit\VOC2007\JPEGImages\的字符数，-4表示去掉.jpg
            #F:\pycharm\YOLOv2_ship_pytorch\data\Seaships_7000\VOCdevkit\VOC2007\JPEGImages\006971.jpg
            ...
            keep = py_cpu_nms(pt4, 0.45)  #得到的是框的索引  0.5  0.45
                    # pt_new = pt3
            print(keep)
            ...
            cv2.imwrite('F:\pycharm\YOLOv2_ship_pytorch_improved_DIoU\show_ship\%s.jpg' % (name), fd)