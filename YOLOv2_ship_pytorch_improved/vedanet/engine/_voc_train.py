    ...
    def train_batch(self):
        ...
        for ii in range(self.nloss):
            ...
            if self.classes > 1:
                ...
                #INFO:311 # 0: Loss:9.21874 (Coord:3.45 Conf:5.75 Cls:0.02)
            else:
                log.info(f'{self.batch} # {ii}: Loss:{round(tot, 5)} (Coord:{round(coord, 2)} Conf:{round(conf, 2)})')
              #
            if ii == 0:
                with open("train_loss_0.txt", "a") as f:
                    f.write('%.5f' % (tot) + '\n')
            else:
                with open("train_loss_1.txt", "a") as f:
                     f.write('%.5f'% (tot) + '\n')

        if self.classes > 1:
            log.info(f'{self.batch} # All : Loss:{round(all_tot, 5)} (Coord:{round(all_coord, 2)} Conf:{round(all_conf, 2)} Cls:{round(all_cls, 2)})')
        # All : Loss:9.21874 (Coord:3.45 Conf:5.75 Cls:0.02)
        else:
            log.info(f'{self.batch} # All : Loss:{round(all_tot, 5)} (Coord:{round(all_coord, 2)} Conf:{round(all_conf, 2)})')

        self.train_loss = [{'tot': [], 'coord': [], 'conf': [], 'cls': []} for _ in range(self.nloss)]
        with open("train_loss_total.txt", "a") as f:
            f.write('%.5f'% (all_tot) + '\n')



        if self.batch % self.backup_rate == 0:   # backup_steps: [1000, 60000]   backup_rates: [10000, 200]   backup_interval: 200
        ...
        if self.batch % self.resize_rate == 0:
            if self.batch + 175 >= self.max_batches:   #200   275    27300
                finish_flag = True
            else:
                finish_flag = False
